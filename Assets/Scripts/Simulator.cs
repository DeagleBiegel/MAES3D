...
public class Simulator : MonoBehaviour{
    ...
    private void Start() {
        ...
        ApplySettings(); //manger timescale
        SetupSimulation();

        _jsonWriter = new JsonWriter2(settingsList[currentSettingsIndex], _simulation.Map.ExplorableTiles, settingsList[currentSettingsIndex].seed);
    ...
    void FixedUpdate() {
        ...
        if (_simulation.ExplorationManager.ExploredRatio >= 99.7f || elapsedTime > SimulationSettings.duration) {
            Debug.Log($"Ratio: {_simulation.ExplorationManager.ExploredRatio}");
            Debug.Log($"Time:  {elapsedTime} - {SimulationSettings.duration}");
        ...
        if (!finishedExploring) {
            ...
            if (currentSettingsIndex < settingsList.Count) {
                ...
                if (settingsList[currentSettingsIndex - 1].algorithm == SimulationSettings.algorithm &&
                    ...
                    _jsonWriter.InitTest(_simulation.Map.ExplorableTiles, settingsList[currentSettingsIndex].seed);
                    _jsonWriter.AddData(0, 0, false);
                }
                else {
                    _jsonWriter.EndFile();
    ...
    private void ApplySettings() {
        ...
        UnityEngine.Random.InitState(current.seed);
